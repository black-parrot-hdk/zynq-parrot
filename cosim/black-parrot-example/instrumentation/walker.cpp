  ...
  switch(ex->VpiType()) {
    ...
    case vpiIndexedPartSelect: {
      ...
      for(auto sel_net : *sel_nets)
        nets->insert((sel_net.back() == ']') ? rtrim(sel_net, '[') : sel_net);
      break;
    }
  ...
  for (auto gs : *ga->Gen_scopes()) {
    ...
    if(gs->Parameters() != nullptr) {
      for(auto p : *gs->Parameters()) {
        ...
        if(pr != nullptr) {
          int val = stoi(ltrim(string(pr->VpiValue()), ':'));
...
int main(int argc, const char** argv) {
  ...
  if (the_design) {
    ...
    for (auto m : *udesign->TopModules()) {
      ...
      ofstream f(outputDir / (ltrim(top->dname, '@') + ".sigs"));
      for(auto c : *covs)
