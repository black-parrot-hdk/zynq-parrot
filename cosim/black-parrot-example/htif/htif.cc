...
void htif_t::read_chunk(addr_t taddr, size_t len, void* dst)
{
  ...
  uint64_t data = zpl->axil_read(base_addr + taddr);
  if(len == 8) {
    uint64_t datah = zpl->axil_read(base_addr + taddr + 4);
    data = data | (datah << 32);
  }
...
void htif_t::write_chunk(addr_t taddr, size_t len, const void* src)
{
  ...
  zpl->axil_write(base_addr + taddr, data, 0xf);
  zpl->axil_write(base_addr + taddr + 4, (data >> 32), 0xf);
}
