...
void *device_poll(void *vargp) {
  ...
  while (1) {
    ...
    if (zpl->axil_read(GP0_PL_TO_PS_FIFO_CTRS + gp0_addr_base) != 0) {
      decode_bp_output(zpl, zpl->axil_read(GP0_PL_TO_PS_FIFO_DATA + gp0_addr_base));
    }
...
extern "C" void cosim_main(char *argstr) {
  ...
  unsigned long long minstret_start = get_counter_64(zpl, GP0_MINSTRET + gp0_addr_base);
  ...
  unsigned long long minstret_stop = get_counter_64(zpl, GP0_MINSTRET + gp0_addr_base);
