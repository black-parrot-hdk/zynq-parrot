...
extern "C" void cosim_main(char *argstr) {
  ...
  printf("ps.cpp: successfully wrote and read registers in bsg_zynq_shell "
         "(verified ARM GP0 connection)\n");
#ifdef FPGA
  data = zpl->axil_read(0x4 + GP0_ADDR_BASE);
  if (data == 0) {
    printf("ps.cpp: CSRs do not contain a DRAM base pointer; calling allocate "
           "dram with size %d\n",
           allocated_dram);
  ...
  if (argc == 1) {
    printf("No nbf file specified, sleeping for 2^31 seconds (this will hold "
           "onto allocated DRAM)\n");
    sleep(1 << 31);
    exit(0);
  }
  ...
  printf("ps.cpp: attempting to read mtime reg in BP CFG space, should "
  ...
  printf("ps.cpp: attempting to read and write mtime reg in BP CFG space "
...
*/
  ...
  printf("ps.cpp: attempting to write L2 %d times over %d MB (testing ARM GP1 "
         "and HP0 connections)\n",
  ...
  printf("ps.cpp: DIRECT access from ARM to DDR (some L1/L2 coherence "
         "mismatches expected) %d matches, %d mismatches, %f\n",
         matches, mismatches, ((float)matches) / (float)(mismatches + matches));
#endif

  printf("ps.cpp: attempting to read L2 %d times over %d MB (testing ARM GP1 "
         "and HP0 connections)\n",
         num_times * outer, (allocated_dram) >> 20);
  for (int s = 0; s < outer; s++)
    for (int t = 0; t < num_times; t++)
  ...
  printf("ps.cpp: READ access through BP (some L1 coherence mismatch "
  ...
  printf("ps.cpp: BP DRAM USAGE MASK (each bit is 8 MB): "
         "%-8.8x%-8.8x%-8.8x%-8.8x\n",
