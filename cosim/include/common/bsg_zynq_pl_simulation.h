...
protected:
    ...
    std::unique_ptr<axiss<DMA_DATA_WIDTH>> axis;
...
#endif
#ifdef DMA_ENABLE
        axis = std::make_unique<axiss<DMA_DATA_WIDTH>>(
            STRINGIFY(DMA_HIER_BASE));
        co_list.push_back(std::make_unique<coro_t>([=](yield_t &yield) {
            axis->reset(yield);
        }));
#endif
...
public:
    ...
    virtual int dma_read(int32_t* data) {
        return buffer.get()->read(data);
    }
};
