...
class bsg_tag_bitbang {
    ...
    void write_bit(int bit) { zpl->shell_write(shell_addr, (bit & 1), 0xf); }
  ...
  public:
    ...
    bsg_tag_bitbang(bsg_zynq_pl *zpl, uintptr_t shell_addr, int num_clients,
                    int max_len)
        : zpl(zpl), shell_addr(shell_addr), id_len(safe_clog2(num_clients)),
          max_len(max_len) {
        bsg_pr_info("Creating Bitbang Driver: %p %" PRIxPTR " %d %d\n", zpl,
                    shell_addr, num_clients, max_len);
    }
